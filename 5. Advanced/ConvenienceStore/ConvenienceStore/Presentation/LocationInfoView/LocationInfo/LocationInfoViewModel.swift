...
struct LocationInfoViewModel {
    ...
    init(_ viewModel: LocationInfoModel = LocationInfoModel()) {
        // MARK: 네트워크 통신으로 데이터 패칭
        let cvsLocationDataResult = mapCenterPoint
            .flatMapLatest(viewModel.getLocation)
            .share()
        ...
        let selectDetailListItem = detailListItemSelected
            ...
            .map(viewModel.documentToMTMapPoint)
        ...
        detailListCellData = documentData
            .map(viewModel.documentsToCellData)
